...
SET @wallet_addresses = '0x8ee830de0cd81de591de840518e0f87902d01468,
...
0x00135361cc98778d87c214db58d88169217b8145';  -- 钱包地址列表，用逗号分隔
...
-- 先删除可能存在的临时表
DROP TEMPORARY TABLE IF EXISTS target_wallets;
DROP TEMPORARY TABLE IF EXISTS rollback_calculations;

CREATE TEMPORARY TABLE target_wallets (
    wallet_address VARCHAR(255) PRIMARY KEY
);

-- 插入目标地址
INSERT INTO target_wallets (wallet_address) VALUES 
...
-- 备份账户数据
CREATE TABLE IF NOT EXISTS backup_account_rollback AS 
SELECT * FROM account 
WHERE wallet_address IN (SELECT wallet_address FROM target_wallets)
  AND type = 'ECO';

-- 备份奖励记录
CREATE TABLE IF NOT EXISTS backup_reward_rollback AS 
...
CREATE TEMPORARY TABLE rollback_calculations AS
SELECT 
    r.wallet_address,
...
INNER JOIN target_wallets t ON r.wallet_address = t.wallet_address
...
-- 4.1 回滚账户余额
UPDATE account a
INNER JOIN rollback_calculations rc ON a.wallet_address = rc.wallet_address
...
INNER JOIN target_wallets t ON at.wallet_address = t.wallet_address
WHERE at.transaction_type IN ('DYNAMIC_REWARD', 'ADD_NUMBER')
  AND at.`order` IN (
      SELECT DISTINCT `order` 
      FROM purchase_miner_project_reward r
      INNER JOIN target_wallets t2 ON r.wallet_address = t2.wallet_address
      WHERE r.day_time = @target_date
        AND r.reward_type = 'NEW'
  );
...
INNER JOIN rollback_calculations rc ON pmp.wallet_address = rc.wallet_address
...
INNER JOIN target_wallets t ON rl.wallet_address = t.wallet_address
...
DROP TEMPORARY TABLE target_wallets;
DROP TEMPORARY TABLE rollback_calculations;
...
-- 恢复账户数据
UPDATE account a
INNER JOIN backup_account_rollback b ON a.wallet_address = b.wallet_address
...
SELECT * FROM backup_reward_rollback;
*/
